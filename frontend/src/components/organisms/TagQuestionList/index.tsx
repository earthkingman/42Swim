...
import useTagQuestionList from "../../../hooks/useTagQuestionList";
...
const TagQuestionList = ({ hashtagName, hashtagId, ...props }: Props) => {
  const [menu, setMenu] = useState(0);
  const [page, setPage] = useState(1);
  const { question, isLoading, isError } = useTagQuestionList(
    menu,
    page,
    hashtagId
  );
    ...
    return (
      <S.QLWrapper>
        <S.List>
          <S.FilterPanel>
            <S.ResultSpan>
              <S.TagSpan>#{hashtagName}</S.TagSpan> 검색결과 (
              {question?.questionCount}건)
            </S.ResultSpan>
            <Tab size="sm" {...props}>
              <TabItem
                size="sm"
